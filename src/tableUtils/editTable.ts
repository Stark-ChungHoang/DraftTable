...
export const editColumn = ({ editorState, columns, column, block }:any) => {
...
export const editCell = ({ editorState, cell, row, block }:any) => {
  const updatedRow = row.value.map((c:any) => {
    return {
      ...c,
...
export const addColumn = ({ columns, rows, index, block, editorState }:any) => {
  const contentState = editorState.getCurrentContent();
  const entityKey = block.getEntityAt(0);
  if (!entityKey) {
    return editorState;
...
export const removeColumn = ({ columns, rows, index, block, editorState }:any) => {
  const contentState = editorState.getCurrentContent();
  const entityKey = block.getEntityAt(0);
  if (!entityKey) {
    return editorState;
...
export const addRow = ({ columns, rows, index, block, editorState }:any) => {
  const contentState = editorState.getCurrentContent();
  const entityKey = block.getEntityAt(0);
  if (!entityKey) {
    return editorState;
...
export const removeRow = ({ columns, rows, index, block, editorState }:any) => {
  const contentState = editorState.getCurrentContent();
  const entityKey = block.getEntityAt(0);
  if (!entityKey) {
    return editorState;
